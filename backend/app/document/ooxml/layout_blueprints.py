...
def source_layout_blueprint(doc: Any) -> dict[str, Any]:
    ...
    return {"schema_version": 1, "section_sectPr": _xml(doc.sections[0]._sectPr) if doc.sections else None, "paragraph_roles": roles, "metadata_paragraphs": metadata, "tables": {**({"mcq": _table(mcq)} if mcq else {}), **({"answer_table": _table(answer_table)} if answer_table else {})}, "parent_totals_by_depth": parent_totals}
