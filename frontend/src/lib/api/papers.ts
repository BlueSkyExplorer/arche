...
const payload = (v: Partial<PaperInput>) => ({ ...(v.title !== undefined && { title: v.title }), ...(v.subject !== undefined && { subject: v.subject }), ...(v.level !== undefined && { level: v.level }), ...(v.paperDate !== undefined && { paper_date: v.paperDate || null }), ...(v.durationMinutes !== undefined && { duration_minutes: v.durationMinutes }), ...(v.instructions !== undefined && { instructions_json: v.instructions }), ...(v.templateProfileId !== undefined && { template_profile_id: v.templateProfileId }) });
