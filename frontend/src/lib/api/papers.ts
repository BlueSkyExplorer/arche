...
export type PaperQuestion = { id?: string; question_id: string; position: number; display_number?: string; question: Question };
...
export function putSectionQuestions(token: string, paperId: string, sectionId: string, questions: { question_id: string }[]) { return apiFetch<PaperSection>(`/api/v1/papers/${encodeURIComponent(paperId)}/sections/${encodeURIComponent(sectionId)}/questions`, token, { method: "PUT", body: JSON.stringify(questions) }); }
